...
// This executes queries against in-memory table data and returns results
function executeSQL(query: string, tables: TableData[]): { columns: string[]; rows: (string | number | null)[][] } | { error: string } {
  try {
    const normalizedQuery = query.trim().toLowerCase().replace(/\s+/g, ' ')
    
    // Create a table lookup
    const tableMap = new Map<string, TableData>()
    tables.forEach(t => tableMap.set(t.tableName.toLowerCase(), t))
    ...
    const mainTableName = selectMatch[2].trim()
    const remainder = selectMatch[3] || ''
    
    const mainTable = tableMap.get(mainTableName)
    if (!mainTable) {
      return { error: `Table '${mainTableName}' not found` }
    }
    
    // Handle JOIN
    let joinedData: { columns: string[]; rows: (string | number | null)[][] } = {
      columns: mainTable.columns.map(c => c.name),
      rows: [...mainTable.rows]
    }
    
    // Parse JOINs
    const joinMatches = remainder.matchAll(/(left\s+join|right\s+join|inner\s+join|join)\s+(\w+)\s+on\s+(\w+)\.(\w+)\s*=\s*(\w+)\.(\w+)/gi)
    for (const joinMatch of joinMatches) {
      const joinType = joinMatch[1].toLowerCase().includes('left') ? 'left' : 
                       joinMatch[1].toLowerCase().includes('right') ? 'right' : 'inner'
      const joinTableName = joinMatch[2].toLowerCase()
      const leftTable = joinMatch[3].toLowerCase()
      const leftCol = joinMatch[4].toLowerCase()
      const rightTable = joinMatch[5].toLowerCase()
      const rightCol = joinMatch[6].toLowerCase()
      ...
      if (joinType === 'inner' || joinType === 'left') {
        for (const row of joinedData.rows) {
          ...
          if (!matched && joinType === 'left') {
            newRows.push([...row, ...joinTable.columns.map(() => null)])
          }
        }
      }
      
      joinedData = { columns: newColumns, rows: newRows }
    }
    
    // Parse WHERE clause
    let filteredRows = [...joinedData.rows]
    const whereMatch = remainder.match(/where\s+(.+?)(?:\s+group\s+|\s+order\s+|\s+having\s+|\s+limit\s+|$)/i)
    if (whereMatch) {
      const whereClause = whereMatch[1].trim()
      filteredRows = filterRows(whereClause, joinedData.columns, filteredRows, tableMap, mainTableName)
    }
    
    // Parse GROUP BY
    const groupByMatch = remainder.match(/group\s+by\s+(.+?)(?:\s+having\s+|\s+order\s+|\s+limit\s+|$)/i)
    let groupedData: { columns: string[]; rows: (string | number | null)[][] } | null = null
    
    if (groupByMatch) {
      const groupCols = groupByMatch[1].split(',').map(c => c.trim().toLowerCase())
      const groupIndices = groupCols.map(gc => joinedData.columns.findIndex(c => c.toLowerCase() === gc))
      ...
      const selectParts = columnsStr.split(',').map(p => p.trim())
      ...
      for (const [, groupRows] of groups) {
        ...
        for (const part of selectParts) {
          ...
          if (aggMatch) {
            ...
            const colIdx = col === '*' ? -1 : joinedData.columns.findIndex(c => c.toLowerCase() === col.toLowerCase())
            
            let value: number = 0
            if (func === 'count') {
              value = groupRows.length
            } else if (colIdx >= 0) {
              const nums = groupRows.map(r => Number(r[colIdx])).filter(n => !isNaN(n))
              if (func === 'sum') value = nums.reduce((a, b) => a + b, 0)
              else if (func === 'avg') value = nums.length ? nums.reduce((a, b) => a + b, 0) / nums.length : 0
    ...
    let selectedRows: (string | number | null)[][] = []
    
    if (groupedData) {
      selectedColumns = groupedData.columns
      selectedRows = groupedData.rows
    } else if (columnsStr === '*') {
      selectedColumns = [...joinedData.columns]
      selectedRows = filteredRows.map(r => [...r])
    } else {
      // Handle DISTINCT
      const isDistinct = columnsStr.toLowerCase().startsWith('distinct ')
      const colsStr = isDistinct ? columnsStr.substring(9).trim() : columnsStr
      
      // Handle aggregates without GROUP BY
      const selectParts = colsStr.split(',').map(p => p.trim())
      const hasAggregate = selectParts.some(p => /^(count|sum|avg|max|min)\s*\(/i.test(p))
      ...
      if (hasAggregate && !groupByMatch) {
        ...
        for (const part of selectParts) {
          ...
          if (aggMatch) {
            ...
            if (func === 'count') {
              ...
              const nums = filteredRows.map(r => Number(r[colIdx])).filter(n => !isNaN(n))
              if (func === 'sum') value = nums.reduce((a, b) => a + b, 0)
              else if (func === 'avg') value = nums.length ? nums.reduce((a, b) => a + b, 0) / nums.length : 0
              else if (func === 'max') value = Math.max(...nums)
              else if (func === 'min') value = Math.min(...nums)
            ...
            resultRow.push(func === 'avg' ? Math.round(value * 100) / 100 : value)
            selectedColumns.push(aliasMatch ? aliasMatch[1] : func)
          }
        }
        ...
        for (const part of selectParts) {
          const aliasMatch = part.match(/(.+?)\s+as\s+(\w+)/i)
          const colName = aliasMatch ? aliasMatch[1].trim() : part
          const alias = aliasMatch ? aliasMatch[2] : colName
        ...
        for (const row of filteredRows) {
          const selectedRow: (string | number | null)[] = []
          for (const part of selectParts) {
            const aliasMatch = part.match(/(.+?)\s+as\s+(\w+)/i)
            const colName = aliasMatch ? aliasMatch[1].trim() : part
            const colIdx = joinedData.columns.findIndex(c => c.toLowerCase() === colName.toLowerCase())
            if (colIdx >= 0) {
              selectedRow.push(row[colIdx])
            }
          }
    ...
    // Parse ORDER BY
    const orderMatch = remainder.match(/order\s+by\s+(.+?)(?:\s+limit\s+|$)/i)
    if (orderMatch) {
      const orderClauses = orderMatch[1].split(',').map(c => c.trim())
      for (const clause of orderClauses.reverse()) {
        const [col, dir] = clause.split(/\s+/)
        const colIdx = selectedColumns.findIndex(c => c.toLowerCase() === col.toLowerCase())
        if (colIdx >= 0) {
    ...
    if (limitMatch) {
      selectedRows = selectedRows.slice(0, parseInt(limitMatch[1]))
    }
    
    return { columns: selectedColumns, rows: selectedRows }
  } catch (e) {
    return { error: `SQL execution error: ${e instanceof Error ? e.message : 'Unknown error'}` }
  }
}

// Helper function to filter rows based on WHERE clause
function filterRows(
  whereClause: string, 
  columns: string[], 
  rows: (string | number | null)[][], 
  ...
  if (notInMatch) {
    ...
    const subCol = notInMatch[2]
    const subTable = tableMap.get(notInMatch[3].toLowerCase())
    
    if (subTable) {
      const subColIdx = subTable.columns.findIndex(c => c.name.toLowerCase() === subCol.toLowerCase())
  ...
  // Handle IN subquery
  const inMatch = whereClause.match(/(\w+)\s+in\s*\(\s*select\s+(\w+)\s+from\s+(\w+)/i)
  if (inMatch) {
    const col = inMatch[1]
    const subCol = inMatch[2]
    const subTable = tableMap.get(inMatch[3].toLowerCase())
  ...
  return rows.filter(row => {
    return conditions.every(cond => {
      ...
      if (eqMatch) {
        const col = eqMatch[1]
        let val: string | number = eqMatch[2].trim()
      ...
      // Handle > comparison
      const gtMatch = cond.match(/(\w+)\s*>\s*(\d+)/i)
      if (gtMatch) {
        const col = gtMatch[1]
        const val = Number(gtMatch[2])
        const colIdx = columns.findIndex(c => c.toLowerCase() === col.toLowerCase())
        if (colIdx >= 0) {
          return Number(row[colIdx]) > val
        }
      }
      
      // Handle < comparison
      const ltMatch = cond.match(/(\w+)\s*<\s*(\d+)/i)
      if (ltMatch) {
        const col = ltMatch[1]
      ...
      // Handle <= comparison
      const lteMatch = cond.match(/(\w+)\s*<=\s*(\d+)/i)
      if (lteMatch) {
      ...
      const neMatch = cond.match(/(\w+)\s*(?:!=|<>)\s*['"]?([^'"]+)['"]?/i)
      if (neMatch) {
        const col = neMatch[1]
        let val: string | number = neMatch[2].trim()
        const colIdx = columns.findIndex(c => c.toLowerCase() === col.toLowerCase())
        if (colIdx >= 0) {
          return String(row[colIdx]).toLowerCase() !== String(val).toLowerCase()
        }
      }
      
      // Handle LIKE comparison
      ...
      if (likeMatch) {
        ...
        if (colIdx >= 0) {
          ...
          return regex.test(String(row[colIdx]))
        }
      }
...
// Compare two result sets for equality (order-insensitive for most cases)
function compareResults(
  ...
  // Normalize and sort rows for comparison (unless order matters)
  const normalizeRow = (row: (string | number | null)[]) => 
    row.map(v => v === null ? 'NULL' : String(v).toLowerCase()).join('|')
  
  const actualNormalized = actual.rows.map(normalizeRow)
  const expectedNormalized = expected.rows.map(normalizeRow)
...
runTestCases: (code) => {
    ...
    // Execute the query against each test case's data and compare outputs
    return challenge.testCases.map(testCase => {
      // Execute the user's query against the test case data
      const result = executeSQL(code, testCase.tableData)
      ...
      const expected = {
        ...
        rows: testCase.expectedOutput
      }
      
      // Check if results match (order doesn't matter for most queries)
      const orderMatters = code.toLowerCase().includes('order by')
      const passed = compareResults(result, expected, orderMatters)
      
      return {
        testCaseId: testCase.id,
        passed,
        actualOutput: result.rows,
